            ...
            <div v-if="live.length > 0">
              <template v-for="item in live" :key="item.id">
                <div class="live">
                  <l-icon />
                  <a
                    :href="item.channel.yt_channel_id ? 'https://www.youtube.com/channel/' + item.channel.yt_channel_id : 'https://live.bilibili.com/' + item.channel.bb_space_id"
                    class="live-title"
                    target="_blank"
                  >
                    {{item.title}}
                    <img
                      :src="item.channel.yt_channel_id ? require('../assets/image/youtube-fill.png') : require('../assets/image/bilibili-fill.png')"
                    />
                  </a>
                </div>
              </template>
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      axios.get(`https://api.konkon.icu/v1/live?channel_id=${Setting.live}`)
        .then(res => {
          if (res.status === 200) {
    ...
    const tip = computed(() => {
      if (noLive.value) return t('live.noLive')
      if (error.value) return t('live.error')
