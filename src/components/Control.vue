<template>
  <div class="control">
    <div class="playing">{{nowPlay ? nowPlay : '当前没有播放哦~'}}</div>
    <div class="btn">
      ...
      <div class="icon" @click="overlapChange" :class="{'icon-active': overlap}">
      ...
      <div class="icon" @click="autoRandomChange" :class="{'icon-active': autoRandom}">
      ...
      <div class="icon" @click="loopChange" :class="{'icon-active': loop}">
...
export default {
  setup () {
    ...
    const overlapChange = () => {
      autoRandom.value = false
      loop.value = false
      overlap.value = !overlap.value
      mitt.emit('setting', {
        overlap: overlap.value,
        autoRandom: autoRandom.value,
        loop: loop.value
      })
    }
    ...
    const autoRandomChange = () => {
      overlap.value = false
      loop.value = false
      autoRandom.value = !autoRandom.value
      mitt.emit('setting', {
        overlap: overlap.value,
        autoRandom: autoRandom.value,
        loop: loop.value
